...
class SimpleVQAutoEncoder(nn.Module):
    def __init__(self, encoder, decoder, h_dim=256, num_quantizers=2, codebook_size=1024):
        """
        Args:
    ...
    def forward(self, x):
        # Encode
        z = self.encoder(x)  # Shape: [batch_size, channels, seq_len]
        z_flat = rearrange(z, "b c l -> b l c")  # Flatten for vector quantization
        ...
        print(vq_out.shape)
        vq_out = rearrange(vq_out, "b l c -> b c l")
    ...
    def compute_loss(self, x, alpha=10):
        ...
        Args:
            x (torch.Tensor): Input batch of audio signals.
    ...
    def get_latent_vector(self, x):
        ...
        z_flat = rearrange(z, "b c l -> b l c")
